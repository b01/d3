...
use const \kshabazz\d3a\ATTRIBUTE_MAP_FILE;
...
class HeroTest extends \PHPUnit_Framework_TestCase
{
	...
	public function test_retrrieving_items()
	{
		...
		$this->assertEquals($fixtures['head'], $tooltipParams['head']['tooltipParams'],
			'Invalid head item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['torso'], $tooltipParams['torso']['tooltipParams'],
			'Invalid torso item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['feet'], $tooltipParams['feet']['tooltipParams'],
			'Invalid feet item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['hands'], $tooltipParams['hands']['tooltipParams'],
			'Invalid hands item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['shoulders'], $tooltipParams['shoulders']['tooltipParams'],
			'Invalid shoulders item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['legs'], $tooltipParams['legs']['tooltipParams'],
			'Invalid legs item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['bracers'], $tooltipParams['bracers']['tooltipParams'],
			'Invalid bracers item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['mainHand'], $tooltipParams['mainHand']['tooltipParams'],
			'Invalid mainHand item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['offHand'], $tooltipParams['offHand']['tooltipParams'],
			'Invalid offHand item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['waist'], $tooltipParams['waist']['tooltipParams'],
			'Invalid waist item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['rightFinger'], $tooltipParams['rightFinger']['tooltipParams'],
			'Invalid rightFinger item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['leftFinger'], $tooltipParams['leftFinger']['tooltipParams'],
			'Invalid leftFinger item returned from Model_GetHero::itemHashes property.'
		);

		$this->assertEquals($fixtures['neck'], $tooltipParams['neck']['tooltipParams'],
