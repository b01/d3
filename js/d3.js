...
D3.Tooltips = { // Reminder: Keep in sync with the equivalent code in tooltips.js
	...
	init: function() {
		// data-d3tooltip
		Tooltip.bind('[data-d3tooltip]', false, function() {

			if(this.rel == 'np') {
				return;
			}

			D3.Tooltips.show(this, Core.baseUrl + '/tooltip/' + $(this).data('d3tooltip'))
		});

		// Automatic tooltips for links across the site
		Tooltip.bind('a', false, function() {

			if(this.rel == 'np' || this.getAttribute('data-d3tooltip') || this.getAttribute('data-tooltip')) {
				return;
			}

			D3.Tooltips.show(this, D3.Tooltips.parseUrl(this));
		});
	},
